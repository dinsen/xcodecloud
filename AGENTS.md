...
Backend reads `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASS`, `DB_NAME`, and optional `XCC_WEBHOOK_SECRET`. `status.php` supports optional `?appId=<ASC_APP_ID>` to scope counts to one app.
